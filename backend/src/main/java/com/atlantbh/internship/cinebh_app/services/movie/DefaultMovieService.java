...
public class DefaultMovieService implements MovieService {
    ...
    public Page<Movie> getAllCurrentMovies(Pageable pageable) {
        return movieRepository.findAll(where(startDateLessThanNow()).and(endDateGreaterThanNow()), pageable);
    }

    @Override
    public Page<Movie> getAllUpcomingMovies(Pageable pageable) {
        return movieRepository.findAll(where(startDateGreaterThanNow()).and(endDateGreaterThanNow()), pageable);
    }
}
