...
export default function AuthSidebarContextProvider({ children }: PropsWithChildren) {
  ...
  return (
    <AuthSidebarContext.Provider
      value={{ openAuthSidebar }}
    >
      <AuthenticationSidebar isOpen={isOpen} setIsOpen={setIsOpen} onLogin={onLogin} />
      {children}
    </AuthSidebarContext.Provider>
  );
}
