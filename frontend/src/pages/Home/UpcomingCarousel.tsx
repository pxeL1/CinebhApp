...
export default function UpcomingCarousel() {
    ...
    const { page, loading, error } = useFetchPage<Movie>("/movie/upcoming", pageNumber, 4);
    ...
    return (
        <MediaCarousel title='Upcoming Movies' nextPage={nextPage} prevPage={prevPage} range={((page.number + 1) + (page.number * page.size - page.number)) + " - " + ((page.number + 1) + (page.number * page.size - page.number) + (page.numberOfElements - 1))} totalElements={page.totalElements} nextActive={!page.last} prevActive={!page.first}>
